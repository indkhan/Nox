...
| F4 | IMPLEMENTED (unit-verified; live pending) | R7 | 17 focused regressions in `extension/tests/mcp-epoch-f4.test.ts` failed on the reviewed revision and pass now; `pnpm test` 774 passed/7 skipped, `typecheck`/`build` pass |
| F5 | IMPLEMENTED (unit-verified; live BLOCKED) | All; C01–C17 | 11 integrated R1–R7 regressions in `extension/tests/writes/epoch-f5.test.ts` plus 6 publish-gate regressions in `scripts/publish-gate.test.mjs` pass; `pnpm test` 785 passed/7 skipped, `typecheck`/`build`/bridge harness pass, 23 root tests pass, both audits 0 advisories, `node scripts/release-smoke.mjs` automated PASS with `--publish-gate` BLOCKED; live C01–C17 BLOCKED without an authorized scratch scope |
