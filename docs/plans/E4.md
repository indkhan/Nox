...
| `executor.ts` | Executes `item/tool/call`: capability check → scheduler → untrusted wrap → truncate → journal hook; per-turn step budget; MCP errors become model-readable strings |
...
| `loop.ts` | Orchestrates: ensure thread (start/resume), send turn, stream normalized events into callbacks, enforce timeout, cancellation via `turn/interrupt`, one transparent bridge-reconnect retry |
...
4. **Cancellation leaves consistent state**: interrupt → done(interrupted) →
   executor counters reset; no half-applied writes exist yet (writes come in E6).
